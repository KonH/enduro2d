    ...
    SECTION("update_texture"){
        ...
        if ( modules::is_initialized<render>() ) {
            ...
            //if ( r.device_capabilities().dxt5_compression_supported ) // TODO: wait for android branch
            {
                ...
                //    make_write_file(path::combine(resources, "result.png"), false)));
            }

            /*if ( r.device_capabilities().pvrtc_compression_supported ) // TODO: wait for android branch
            {
                str resources;
                REQUIRE(filesystem::extract_predef_path(
                    resources,
                ...
                image src;
                REQUIRE(images::try_load_image(src, make_read_file(path::combine(resources, "bin/images/ship_pvrtc_4bpp_rgba.pvr"))));
            
                texture_ptr tex = r.create_texture(src.size(), pixel_declaration::pixel_type::rgba_pvrtc4);
                REQUIRE(tex != nullptr);
                REQUIRE_NOTHROW(r.update_texture(tex, src, v2u(0,0)));
